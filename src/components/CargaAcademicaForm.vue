...
export default {
  ...
  methods: {
    ...
    back: function () {
      this.$router.push("/carga_academica");
    },

    create: function (payload) {
      client
        .post("/v1/carga_academica", payload)
        .then(() => {
          console.log(payload);

          this.message = "Persona Agragado!";
          this.showMessage = true;
          this.$router.push("/carga_academica?msg="+this.message);
        })
        .catch((error) => {
          this.message = error;
          this.showMessage = true;
    ...
    update: function (payload, id) {
      client
        .put(`/v1/carga_academica/${id}`, payload)
        .then(() => {
          console.log(payload);
